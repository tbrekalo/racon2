...
Overlap::Overlap(uint64_t a_id, uint64_t b_id, double, uint32_t, uint32_t a_rc,
                 uint32_t a_begin, uint32_t a_end, uint32_t a_length,
                 uint32_t b_rc, uint32_t b_begin, uint32_t b_end,
                 uint32_t b_length)
    : q_name_(),
      q_id_(a_id - 1),
      q_begin_(a_begin),
...
Overlap::Overlap(const char* q_name, uint32_t q_name_length, uint32_t q_length,
                 ...
                 uint32_t t_begin, uint32_t t_end, uint32_t, uint32_t, uint32_t)
...
Overlap::Overlap(const char* q_name, uint32_t q_name_length, uint32_t flag,
                 ...
                 uint32_t, const char* cigar, uint32_t cigar_length,
                 const char*, uint32_t, uint32_t, uint32_t, const char*,
...
std::string_view Overlap::cigar(
    std::span<const std::unique_ptr<Sequence>> sequences) {
  if (!is_transmuted_) {
    throw std::runtime_error(
        "[racon::Overlap::cigar] error: "
        "overlap is not transmuted!\n");
  } else if (!cigar_.empty()) {
    return cigar_;
  }

  const char* q =
      !strand_ ? &(sequences[q_id_]->data()[q_begin_])
               : &(sequences[q_id_]->reverse_complement()[q_length_ - q_end_]);
  const auto q_length = q_end_ - q_begin_;

  const char* t = &(sequences[t_id_]->data()[t_begin_]);
  const auto t_length = t_end_ - t_begin_;

  EdlibAlignResult result = edlibAlign(
      q, q_length, t, t_length,
