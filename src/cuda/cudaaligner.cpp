...
std::unique_ptr<CUDABatchAligner> createCUDABatchAligner(
    uint32_t max_bandwidth, uint32_t device_id, int64_t max_gpu_memory) {
  return std::unique_ptr<CUDABatchAligner>(
      new CUDABatchAligner(max_bandwidth, device_id, max_gpu_memory));
}

CUDABatchAligner::CUDABatchAligner(uint32_t max_bandwidth, uint32_t device_id,
  ...
  aligner_ = create_aligner(AlignmentType::global_alignment, max_bandwidth,
                            stream_, device_id, max_gpu_memory);
}
...
bool CUDABatchAligner::addOverlap(
    Overlap* overlap, std::vector<std::unique_ptr<Sequence>>& sequences) {
  const char* q =
      !overlap->strand_
          ? &(sequences[overlap->q_id_]->data()[overlap->q_begin_])
          : &(sequences[overlap->q_id_]
                  ->reverse_complement()[overlap->q_length_ - overlap->q_end_]);
  ...
  // Hence, what is treated as target in edlib is query in cudaaligner and vice
  // versa.
...
void CUDABatchAligner::alignAll() { aligner_->align_all(); }
...
void CUDABatchAligner::generate_cigar_strings() {
  ...
  if (overlaps_.size() != alignments.size()) {
    throw std::runtime_error(
        "Number of alignments doesn't match number of overlaps in "
        "cudaaligner.");
  }
