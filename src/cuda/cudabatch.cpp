...
std::unique_ptr<CUDABatchProcessor> createCUDABatch(
    uint32_t max_window_depth, uint32_t device, size_t avail_mem, int8_t gap,
    int8_t mismatch, int8_t match, bool cuda_banded_alignment) {
  return std::unique_ptr<CUDABatchProcessor>(
      new CUDABatchProcessor(max_window_depth, device, avail_mem, gap, mismatch,
                             match, cuda_banded_alignment));
}

CUDABatchProcessor::CUDABatchProcessor(uint32_t max_window_depth,
                                       uint32_t device, size_t avail_mem,
                                       int8_t gap, int8_t mismatch,
                                       int8_t match, bool cuda_banded_alignment)
    : windows_(), seqs_added_per_window_() {
  ...
  BatchConfig batch_config(
      1023, max_window_depth, 256,
      cuda_banded_alignment ? BandMode::static_band : BandMode::full_band);

  cudapoa_batch_ =
      create_batch(device, stream_, avail_mem, OutputType::consensus,
                   batch_config, gap, mismatch, match);
}
...
bool CUDABatchProcessor::addWindow(std::shared_ptr<Window> window) {
  ...
  std::vector<std::vector<int8_t>> all_read_weights(num_seqs,
                                                    std::vector<int8_t>());
  ...
  convertPhredQualityToWeights(qualities.first, qualities.second,
                               all_read_weights[0]);
  Entry e = {seq.first, all_read_weights[0].data(),
  ...
  for (uint32_t j = 1; j < num_seqs; j++) {
    ...
    Entry p = {seq.first, all_read_weights[i].data(),
               static_cast<int32_t>(seq.second)};
  ...
  for (uint32_t i = 1; i < entry_status.size(); i++) {
    if (entry_status[i] == StatusType::exceeded_maximum_sequence_size) {
      long_seq++;
      continue;
    } else if (entry_status[i] ==
               StatusType::exceeded_maximum_sequences_per_poa) {
      skipped_seq++;
      continue;
...
void CUDABatchProcessor::convertPhredQualityToWeights(
    const char* qual, uint32_t qual_length, std::vector<int8_t>& weights) {
  weights.clear();
    ...
    if (output_status.at(i) != StatusType::success) {
      ...
      if (window->sequences_.size() < 3) {
        ...
        if (window->type_ == WindowType::kTGS) {
          ...
          for (; begin < static_cast<int32_t>(window->consensus_.size());
               ++begin) {
          ...
          if (begin >= end) {
            ...
            window->consensus_ =
                window->consensus_.substr(begin, end - begin + 1);
