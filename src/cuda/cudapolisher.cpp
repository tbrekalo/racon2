...
CUDAPolisher::CUDAPolisher(std::unique_ptr<bioparser::Parser<Sequence>> sparser,
                           ...
                           PolisherType type, uint32_t window_length,
                           double quality_threshold, double error_threshold,
                           bool trim, int8_t match, int8_t mismatch, int8_t gap,
                           uint32_t num_threads, uint32_t cudapoa_batches,
                           bool cuda_banded_alignment,
                           uint32_t cudaaligner_batches,
                           uint32_t cudaaligner_band_width)
    : Polisher(std::move(sparser), std::move(oparser), std::move(tparser), type,
  ...
  std::cerr << "Using " << num_devices_ << " GPU(s) to perform polishing"
            << std::endl;
...
void CUDAPolisher::find_overlap_breaking_points(
  ...
  if (cudaaligner_batches_ >= 1) {
    ...
    auto fill_next_batch =
        [&mutex_overlaps, &next_overlap_index, &overlaps,
         this](CUDABatchAligner* batch) -> std::pair<uint32_t, uint32_t> {
      batch->reset();
    ...
    auto process_batch = [&fill_next_batch, &logger_step, &log_bar_idx,
                          &log_bar_idx_prev, &window_idx, &mutex_log_bar_idx,
                          this](CUDABatchAligner* batch) -> void {
      while (true) {
        ...
        if (batch->hasOverlaps()) {
          ...
          // logging bar
          {
            ...
            if (log_bar_idx == log_bar_idx_prev) {
              continue;
            } else if (logger_step != 0 &&
                       log_bar_idx <
                           static_cast<int32_t>(RACON_LOGGER_BIN_SIZE)) {
              logger_->bar(
    ...
    for (int32_t device = 0; device < num_devices_; device++) {
      ...
      const size_t free_usable_memory =
          static_cast<float>(free) * 90 / 100;  // Using 90% of available memory
      const int64_t usable_memory_per_aligner =
      ...
      std::cerr << "GPU " << device << ": Aligning with band width "
                << max_bandwidth << std::endl;
      for (uint32_t batch = 0; batch < cudaaligner_batches_; batch++) {
        batch_aligners_.emplace_back(createCUDABatchAligner(
            max_bandwidth, device, usable_memory_per_aligner));
      }
    }

    logger_->log(
        "[racon::CUDAPolisher::initialize] allocated memory on GPUs for "
        "alignment");
    ...
    for (auto& aligner : batch_aligners_) {
      thread_futures.emplace_back(
          thread_pool_->Submit(process_batch, aligner.get()));
    }
    ...
    int64_t missing_overlaps = std::count_if(
        begin(overlaps), end(overlaps),
        [](std::unique_ptr<Overlap> const& o) { return o->cigar().empty(); });

    std::cerr << "Alignment skipped by GPU: " << missing_overlaps << " / "
  ...
  if (cudapoa_batches_ < 1) {
    ...
    for (int32_t device = 0; device < num_devices_; device++) {
      ...
      for (uint32_t batch = 0; batch < cudapoa_batches_; batch++) {
        batch_processors_.emplace_back(
            createCUDABatch(MAX_DEPTH_PER_WINDOW, device, mem_per_batch, gap_,
                            mismatch_, match_, cuda_banded_alignment_));
      }
    }

    logger_->log(
        "[racon::CUDAPolisher::polish] allocated memory on GPUs for polishing");
    ...
    auto fill_next_batch =
        [&mutex_windows, &next_window_index,
         this](CUDABatchProcessor* batch) -> std::pair<uint32_t, uint32_t> {
      batch->reset();
    ...
    auto process_batch = [&fill_next_batch, &logger_step, &log_bar_idx,
                          &mutex_log_bar_idx, &window_idx, &log_bar_idx_prev,
                          this](CUDABatchProcessor* batch) -> void {
      while (true) {
        ...
        if (batch->hasWindows()) {
          ...
          if (results.size() != (range.second - range.first)) {
            throw std::runtime_error(
                "Windows processed doesn't match \
    ...
    for (auto& batch_processor : batch_processors_) {
      thread_futures.emplace_back(
          thread_pool_->Submit(process_batch, batch_processor.get()));
    }
    ...
    for (uint64_t i = 0; i < windows_.size(); ++i) {
      if (window_consensus_status_.at(i) == false) {
        thread_failed_windows.emplace_back(thread_pool_->Submit(
            [&](uint64_t j) -> bool {
              auto it =
                  thread_pool_->thread_map().find(std::this_thread::get_id());
              return window_consensus_status_.at(j) =
                         windows_[j]->generate_consensus(
    ...
    if (thread_failed_windows.size() > 0) {
      logger_->log(
          "[racon::CUDAPolisher::polish] polished remaining windows on CPU");
      logger_->log();
    }
