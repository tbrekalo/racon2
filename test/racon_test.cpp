...
uint32_t calculateEditDistance(const std::string& query,
...
class RaconPolishingTest : public ::testing::Test {
 public:
  void SetUp(const std::string& sequences_path,
             const std::string& overlaps_path, const std::string& target_path,
             racon::PolisherType type, uint32_t window_length,
             double quality_threshold, double error_threshold, int8_t match,
             int8_t mismatch, int8_t gap, uint32_t cuda_batches = 0,
             bool cuda_banded_alignment = false,
             uint32_t cudaaligner_batches = 0) {
    polisher = racon::createPolisher(
        sequences_path, overlaps_path, target_path, type, window_length,
        quality_threshold, error_threshold, true, match, mismatch, gap, 4,
        cuda_batches, cuda_banded_alignment, cudaaligner_batches);
  }

  void TearDown() {}

  void initialize() { polisher->initialize(); }
...
TEST(RaconInitializeTest, PolisherTypeError) {
  EXPECT_DEATH(
      (racon::createPolisher("", "", "", static_cast<racon::PolisherType>(3), 0,
                             0, 0, 0, 0, 0, 0, 0)),
...
TEST(RaconInitializeTest, WindowLengthError) {
  EXPECT_DEATH((racon::createPolisher("", "", "", racon::PolisherType::kC, 0, 0,
                                      0, 0, 0, 0, 0, 0)),
...
TEST(RaconInitializeTest, SequencesPathExtensionError) {
  EXPECT_DEATH(
      (racon::createPolisher("", "", "", racon::PolisherType::kC, 500, 0, 0, 0,
                             0, 0, 0, 0)),
...
TEST(RaconInitializeTest, OverlapsPathExtensionError) {
  EXPECT_DEATH(
      (racon::createPolisher(std::string(TEST_DATA) + "sample_reads.fastq.gz",
                             "", "", racon::PolisherType::kC, 500, 0, 0, 0, 0,
                             0, 0, 0)),
...
TEST(RaconInitializeTest, TargetPathExtensionError) {
  EXPECT_DEATH((racon::createPolisher(
                   std::string(TEST_DATA) + "sample_reads.fastq.gz",
                   std::string(TEST_DATA) + "sample_overlaps.paf.gz", "",
...
TEST_F(RaconPolishingTest, ConsensusWithQualities) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fastq.gz",
        std::string(TEST_DATA) + "sample_overlaps.paf.gz",
        std::string(TEST_DATA) + "sample_layout.fasta.gz",
  ...
  auto parser =
      bioparser::Parser<racon::Sequence>::Create<bioparser::FastaParser>(
          std::string(TEST_DATA) + "sample_reference.fasta.gz");
  auto reference = parser->Parse(-1);
  EXPECT_EQ(reference.size(), 1);

  EXPECT_EQ(1312,
            calculateEditDistance(polished_sequences[0]->reverse_complement(),
                                  reference[0]->data()));
}

TEST_F(RaconPolishingTest, ConsensusWithoutQualities) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fasta.gz",
        std::string(TEST_DATA) + "sample_overlaps.paf.gz",
        std::string(TEST_DATA) + "sample_layout.fasta.gz",
        racon::PolisherType::kC, 500, 10, 0.3, 5, -4, -8);
...
TEST_F(RaconPolishingTest, ConsensusWithQualitiesAndAlignments) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fastq.gz",
        std::string(TEST_DATA) + "sample_overlaps.sam.gz",
        std::string(TEST_DATA) + "sample_layout.fasta.gz",
        racon::PolisherType::kC, 500, 10, 0.3, 5, -4, -8);
...
TEST_F(RaconPolishingTest, FragmentCorrectionWithQualities) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fastq.gz",
        std::string(TEST_DATA) + "sample_ava_overlaps.paf.gz",
        std::string(TEST_DATA) + "sample_reads.fastq.gz",
        racon::PolisherType::kC, 500, 10, 0.3, 1, -1, -1);
...
TEST_F(RaconPolishingTest, FragmentCorrectionWithoutQualitiesFull) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fasta.gz",
        std::string(TEST_DATA) + "sample_ava_overlaps.paf.gz",
        std::string(TEST_DATA) + "sample_reads.fasta.gz",
        racon::PolisherType::kF, 500, 10, 0.3, 1, -1, -1);
...
TEST_F(RaconPolishingTest, FragmentCorrectionWithQualitiesFullMhap) {
  SetUp(std::string(TEST_DATA) + "sample_reads.fastq.gz",
        std::string(TEST_DATA) + "sample_ava_overlaps.mhap.gz",
        std::string(TEST_DATA) + "sample_reads.fastq.gz",
        racon::PolisherType::kF, 500, 10, 0.3, 1, -1, -1);
